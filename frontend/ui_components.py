...
def worklog_card(log: dict, jira_base_url: str = ""):
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            st.markdown(
                f"**<a href='{link}' target='_blank'>{name}</a> {t('ui.logged_hours', hours=log['hours'])}**",
                unsafe_allow_html=True,
            )
            st.markdown(_get_worklog_title(log, jira_base_url))
            st.caption(
                f"**{t('common.project')}:** {log.get('project_name', 'N/A')} | "
