...
from api_client import (
    get_me, fetch_my_leaves, submit_leave_request, 
    fetch_team_leaves, update_leave_status, get_all_employees,
    fetch_org_units, fetch_all_leaves
)
from i18n import t
...
with tabs[0]:
    ...
    with col2:
        ...
        if my_leaves:
            ...
            df_display.columns = [t(f"leaves.{c}") if c in ["type", "start_date", "end_date", "status", "comment"] else c for c in display_cols]
    ...
    if all_company_leaves:
        ...
        with st.expander("🔍 " + t("common.filters"), expanded=False):
            ...
            with f_col1:
                ...
                emp_options = {e.get("display_name") or e.get("full_name") or f"User {e.get('user_id', e.get('id'))}": e.get("user_id") or e.get("id") for e in employees}
            ...
            for leaf in team_leaves:
                with st.container(border=True):
                    ...
                    with c2:
                        ...
                        status_color = "orange" if leaf["status"] == "PENDING" else "green" if leaf["status"] == "APPROVED" else "red"
                        st.markdown(f"{t('common.status')}: **:{status_color}[{t('common.status_' + leaf['status'].lower())}]**")
                    with c3:
    ...
    with tabs[2]:
        ...
        if all_leaves:
            for leaf in all_leaves:
                with st.expander(f"{leaf.get('user_full_name', t('common.unassigned'))} - {t('leaves.' + leaf['type'].lower())} ({leaf['start_date']} {t('common.to').lower()} {leaf['end_date']}) - {t('common.status_' + leaf['status'].lower())}"):
                    with st.form(f"edit_leaf_{leaf['id']}"):
                        ...
                        with c1:
                            ...
                            new_type = st.selectbox(t("common.type"), type_list, index=idx_type, format_func=lambda x: t(f"leaves.{x.lower()}"), key=f"t_{leaf['id']}")
                            ...
                            new_status = st.selectbox(t("common.status"), status_list, index=idx_status, format_func=lambda x: t(f"common.status_{x.lower()}"), key=f"s_{leaf['id']}")
                        with c2:
                            try:
                        ...
                        with c3:
                            new_reason = st.text_area(t("leaves.reason"), value=leaf["reason"] or "", height=68, key=f"r_{leaf['id']}")
                            new_comment = st.text_input(t("leaves.comment"), value=leaf.get("comment") or "", key=f"c_{leaf['id']}")
