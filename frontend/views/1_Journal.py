...
from ui_components import error_state, loading_skeleton, pagination_ui, safe_api_call, worklog_card
...
def _dialog_task_selection():
    """Helper to handle JIRA task selection in the dialog."""
...
def add_worklog_dialog():
    # 1. User Selection
    selected_user_id = _dialog_user_selection(user_role, user_info)
    if not selected_user_id:
        return
    ...
    issue_id = None
    if log_type == t("journal.type_jira"):
        issue_id = _dialog_task_selection()
    ...
    with f_col1:
        ...
        if not p_err:
            ...
            for p in project_list:
                project_options[p["id"]] = {"name": p["name"], "key": p["key"]}
