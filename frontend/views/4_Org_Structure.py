...
from api_client import (
    fetch_org_units, create_org_unit, update_org_unit, delete_org_unit,
    fetch_roles, create_role, delete_role,
    fetch_approval_routes, create_approval_route, delete_approval_route,
    fetch_unit_roles, assign_unit_role, remove_unit_role,
    get_headers, get_all_users, get_employees
)
...
with tab2:
    ...
    with st.expander(t("org.add_unit")):
        ...
        new_parent = st.selectbox(t("org.parent_unit"), options=list(unit_options.keys()), format_func=lambda x: unit_options[x], key="new_unit_parent")
        new_period = st.selectbox(t("org.reporting_period"), ["weekly", "biweekly", "monthly"], format_func=lambda x: t(f"org.period_{x}"), key="new_unit_period")
    ...
    if units:
        ...
        unit_to_edit = st.selectbox(t("org.select_unit"), options=units, format_func=lambda x: x["name"], key="edit_unit_select")
        ...
        with col1:
            ...
            edit_parent = st.selectbox(t("org.parent_unit"), options=list(valid_parents.keys()), index=list(valid_parents.keys()).index(unit_to_edit.get("parent_id")), format_func=lambda x: valid_parents[x], key="edit_unit_parent")
            edit_period = st.selectbox(t("org.reporting_period"), ["weekly", "biweekly", "monthly"], index=["weekly", "biweekly", "monthly"].index(unit_to_edit.get("reporting_period", "weekly")), format_func=lambda x: t(f"org.period_{x}"), key="edit_unit_period")
    ...
    if roles:
        role_to_delete = st.selectbox(t("org.select_role_delete"), options=roles, format_func=lambda x: x["name"], key="delete_role_select")
    ...
    if units and roles:
        selected_unit = st.selectbox(t("org.select_unit_assign"), options=units, format_func=lambda x: x["name"], key="assign_unit")
        ...
        with st.expander(t("org.assign_new_role")):
            user_id = st.selectbox(t("common.employee"), options=[u["id"] for u in users], format_func=lambda x: next((u["full_name"] for u in users if u["id"] == x), ""), key="assign_role_user")
            role_id = st.selectbox(t("common.role"), options=[r["id"] for r in roles], format_func=lambda x: next((r["name"] for r in roles if r["id"] == x), ""), key="assign_role_role")
...
with tab3:
    ...
    if units and roles:
        ...
        with st.form("add_route_form"):
            ...
            req_role = st.selectbox(t("org.required_role"), options=[r["id"] for r in roles], format_func=lambda x: next((rl["name"] for rl in roles if rl["id"] == x), ""), key="wf_step_role")
