...
if data:
    ...
    if view_type == t("dashboard.exec_view"):
        ...
        display_mode = st.radio(t("dashboard.display_mode"), [t("dashboard.list_view"), t("dashboard.comparison_view")], horizontal=True)
        granularity = st.radio(t("dashboard.time_granularity"), [t("dashboard.month"), t("dashboard.releases")], horizontal=True)
        ...
        if display_mode == t("dashboard.list_view"):
            ...
            for team, team_data in display_df.groupby("OrgUnit"):
                rows.append({
                    t("common.employee"): f"**{team}**", t("common.date"): "", t("common.department"): "", t("common.period"): "",
                    t("common.hours"): team_data[t("common.hours")].sum(), t("common.capex"): team_data[t("common.capex")].sum(), t("common.opex"): team_data[t("common.opex")].sum(),
                    "is_header": True
                })
                for _, row in team_data.sort_values(t("common.employee")).iterrows():
                    rows.append({
                        t("common.employee"): row[t("common.employee")], t("common.date"): row[t("common.date")], t("common.department"): row[t("common.department")], 
                        t("common.period"): row[t("common.period")], t("common.hours"): row[t("common.hours")], t("common.capex"): row[t("common.capex")], t("common.opex"): row[t("common.opex")],
                        "is_header": False
                    })
            ...
            final_pivot = final_pivot.rename(columns={"User": t("common.employee"), "Department": t("common.department")})
        ...
        pivot = df.pivot_table(index=["Department", "OrgUnit", "User"], columns="Category", values="Hours", aggfunc="sum", fill_value=0)
        st.dataframe(pivot)
