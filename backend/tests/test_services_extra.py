...
async def test_timesheet_service_extra():
    ...
    with patch("services.timesheet.select"):
        ...
        db.execute.return_value = mock_res
        w = await timesheet_service.get_user_worklogs(
            db, user_id=1, start_date=date(2026, 1, 1), end_date=date(2026, 1, 1)
        )
...
async def test_notification_service_extra():
    ...
    with patch.object(notification_service, "create_notification") as mock_create:
        await notification_service.notify_timesheet_status_change(
            db, user_id=1, approver_id=2, timesheet_id=1, status="APPROVED", period_label="P"
        )
