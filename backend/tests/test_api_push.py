...
async def test_users_search(client: AsyncClient, auth_headers: dict, db: AsyncSession):
    ...
    u1 = User(
        email=f"u_{uuid.uuid4()}@ex.com", full_name=unique_name, hashed_password="pw", role="Employee", is_active=True
    )
    db.add(u1)
    await db.commit()
...
async def test_timesheet_manual_extra(client: AsyncClient, auth_headers: dict, db: AsyncSession, admin_user: User):
    ...
    payload = {"date": str(date.today()), "hours": 1.0, "category": "Development", "description": "D", "issue_id": i.id}
