...
async def setup_control_sheet_data(db: AsyncSession):
    # 1. Create PM
    pm = User(email="pm@example.com", hashed_password=get_password_hash("pass"), full_name="OrgUnit Lead PM", role="PM")
    # 2. Create Admin
    admin = User(
        email="admin@example.com", hashed_password=get_password_hash("pass"), full_name="Super Admin", role="Admin"
    )
    db.add_all([pm, admin])
    await db.flush()
    ...
    # 4. Create Jira Users
    u1 = JiraUser(jira_account_id="user_a", display_name="Alice (OrgUnit A)", org_unit_id=team_a.id, is_active=True)
    u2 = JiraUser(jira_account_id="user_b", display_name="Bob (OrgUnit B)", org_unit_id=team_b.id, is_active=True)
    db.add_all([u1, u2])
    await db.flush()
    ...
    w1 = Worklog(date=date.today(), hours=5.0, jira_user_id=u1.id, category_id=cat.id, type="JIRA")
    w2 = Worklog(date=date.today(), hours=3.0, jira_user_id=u2.id, category_id=cat.id, type="JIRA")
    db.add_all([w1, w2])
    await db.commit()
...
async def test_team_periods_scoping_for_pm(client: AsyncClient, setup_control_sheet_data):
    ...
    response = await client.get(
        f"/api/v1/approvals/team-periods?start_date={monday.isoformat()}&end_date={sunday.isoformat()}", headers=headers
    )
    assert response.status_code == 200
...
async def test_pm_approves_timesheet(client: AsyncClient, setup_control_sheet_data):
    ...
    app_res = await client.post(
        f"/api/v1/approvals/{period_id}/approve", json={"status": "APPROVED", "comment": "Good job"}, headers=pm_headers
    )
