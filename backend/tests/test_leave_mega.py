...
async def test_leave_mega_coverage(client: AsyncClient, db: AsyncSession):
    # 1. Setup Admin
    admin = User(email="admin_mega@ex.com", full_name="Admin Mega", hashed_password=get_password_hash("testpass"), role="Admin", is_active=True)
    db.add(admin)
    await db.commit()
    ...
    # 2. Setup HR
    hr = User(email="hr_mega@ex.com", full_name="HR Mega", hashed_password=get_password_hash("testpass"), role="HR", is_active=True)
    db.add(hr)
    await db.commit()
