...
async def get_holidays(
    start_date: date, end_date: date, db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)
):
...
async def get_all_calendar_events(
    ...
    events = [{"date": h.date, "title": f"🎁 {h.name}", "type": "holiday", "is_holiday": True} for h in holidays]
...
async def get_country(db: AsyncSession = Depends(get_db), current_user=Depends(get_current_user)):
