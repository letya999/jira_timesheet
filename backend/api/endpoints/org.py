...
async def update_employee(
    ...
    result = await db.execute(
        select(JiraUser)
        .where(JiraUser.id == employee_id)
        .options(selectinload(JiraUser.user))
    )
