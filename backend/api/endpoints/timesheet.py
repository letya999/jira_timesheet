...
async def get_timesheet(
    ...
    # Hierarchy filters are complex and require joins.
    # We build the main query and the count query in parallel.
    base_query = select(Worklog)
    count_query = select(func.count(Worklog.id)).select_from(Worklog)

    # Apply joins needed for any potential filter
    # These will be optimized out by the DB if not used in WHERE, but it's safer to have them.
    base_query = base_query.outerjoin(JiraUser, Worklog.jira_user_id == JiraUser.id)\
                           .outerjoin(Issue, Worklog.issue_id == Issue.id)\
                           .outerjoin(Project, Issue.project_id == Project.id)
    count_query = count_query.outerjoin(JiraUser, Worklog.jira_user_id == JiraUser.id)\
                               .outerjoin(Issue, Worklog.issue_id == Issue.id)\
                               .outerjoin(Project, Issue.project_id == Project.id)
    ...
    if any([dept_id, div_id, team_id]) and any([v != 0 for v in [dept_id, div_id, team_id] if v is not None]):
        ...
        if team_id and team_id != 0:
            ...
            # Join from JiraUser -> Team
            base_query = base_query.join(Team, JiraUser.team_id == Team.id)
            ...
            # Join from JiraUser -> Team -> Division
            base_query = base_query.join(Team, JiraUser.team_id == Team.id).join(Division, Team.division_id == Division.id)
            count_query = count_query.join(Team, JiraUser.team_id == Team.id).join(Division, Team.division_id == Division.id)
    ...
    # Now, finalize the main query with eager loading and pagination
    query = base_query.options(
        selectinload(Worklog.jira_user),
        selectinload(Worklog.issue).options(
            selectinload(Issue.project),
            selectinload(Issue.sprints),
            selectinload(Issue.releases)
        )
    )
    
    if sort_order == "asc":
        query = query.order_by(Worklog.source_created_at.asc(), Worklog.id.asc())
    else:
        query = query.order_by(Worklog.source_created_at.desc(), Worklog.id.desc())
    ...
    for wl in worklogs:
        output.append({
            ...
            "source_created_at": wl.source_created_at,
            ...
            "user_id": wl.jira_user.id if wl.jira_user else None,
...
async def create_manual_log(
    ...
    target_jira_user_id = current_user.jira_user_id
    if log_in.user_id and is_privileged:
        target_jira_user_id = log_in.user_id
        
    if not target_jira_user_id:
        raise HTTPException(status_code=400, detail="Target user not linked to any Jira account")
    ...
    db_log = Worklog(
        ...
        jira_user_id=target_jira_user_id,
